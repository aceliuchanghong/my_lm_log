# https://www.milvus-io.com/getstarted/quickstart
...
)  # Replace with your Milvus server address
...
client.create_collection(
    collection_name="hello_milvus", schema=schema, consistency_level="Strong"
)
...
def generate_random_entities(num_entities, dim):
    ...
    for _ in range(num_entities):
        pk = generate_random_string(
            10
        )  # Generate a random primary key string of length 10
        random_value = random.random()  # Generate a random double value
        embeddings = np.random.rand(
            dim
        ).tolist()  # Generate a random float vector of dimension 'dim'
...
print(
    f"Number of entities in Milvus: {insert_result['insert_count']}"
...
result = client.query(
    collection_name="hello_milvus", filter=expr, output_fields=["random", "embeddings"]
)
