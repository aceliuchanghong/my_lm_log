...
snapshot_download('iic/CosyVoice2-0.5B', local_dir='/mnt/data/llch/cosyvoice2')

https://github.com/jianchang512/cosyvoice-api/blob/main/README.md

他妈的,官方文档一坨屎,操了,老子不做了
"""
...
root_dir = "no_git_oic/cosyvoice2"
...
app = Flask(__name__, static_folder=tmp_dir, static_url_path=tmp_dir)
...
def batch(tts_type, outname, params):
    ...
    if tts_type == "tts":
        if sft_model is None:
            sft_model = CosyVoice(
                "pretrained_models/CosyVoice-300M-SFT", load_jit=True, load_onnx=False
            )
...
def audio_speech():
    ...
    try:
        if voice in VOICE_LIST:
            ...
            raise ValueError(f"无效的配音角色名或参考音频路径: {voice}")
