            ...
            if os.path.isfile(item):
                ...
                for folder_name, sub_folders, file_names in os.walk(item):
                    for filename in file_names:
                        ...
                        zip_object.write(
                            file_path,
                            os.path.join(os.path.basename(item), relative_path),
                        )
            else:
...
def pdf_parse(pdf_path: str, progress=gr.Progress()):
    # 文件迁移到脚本目录的.temp
...
def create_app():
    with gr.Blocks() as demo:
        ...
        extract_button.click(
            pdf_parse, inputs=[pdf_input], outputs=[markdown_output, base_path]
        )
        export_button.click(export_zip, inputs=[base_path], outputs=[download_output])
    return demo


if __name__ == "__main__":
    app = create_app()
    app.launch(server_name="0.0.0.0", server_port=6012, share=False)
