    ...
    {{
      ...
      "Details": [
        {{
          ...
          "target_entity": "步骤1中识别的 target_entity 的名称",
          "relationship_description": "给出 source_entity 和 target_entity 的关系描述",
          "relationship_strength": "表示 target_entity 和 target_entity 之间关系强度的数值分数",
...
{{
    "Role": "你是一个出色的秘书,负责对下面提供的数据进行全面总结。",
    "Goal": "给定一个或两个实体,以及与之相关的描述列表。请将所有这些内容合并成一个全面的描述。确保包含所有描述中的信息。如果提供的描述存在矛盾,请解决这些矛盾并提供一个连贯的总结。确保以第三人称书写,并包含实体名称以便用户了解完整上下文。",
    "Data Format": {{
      "实体": "{entity_name}",
...
] = """在上次提取中遗漏了许多 entity。请使用相同的格式将它们添加到下面:
"""

PROMPTS[
    "entiti_if_loop_extraction"
] = """是否还有需要添加的实体,请仅回答YES或NO。
"""
...
{{
  "Role": "你是一个出色的秘书,负责回答关于提供表格中数据的问题。",
...
{{
  "Role": "你是一个出色的秘书,任务是识别用户查询中的高层次和低层次关键词。",
    ...
    {{
      "Example 1": {{
        ...
        "Output": {{
          ...
          "low_level_keywords": ["贸易协议", "关税", "货币兑换量", "进口数量", "出口数量"]
        }}
      }}
    }},
    {{
    ...
    {{
      "Example 3": {{
        ...
        "Output": {{
          ...
          "low_level_keywords": ["入学率", "识字率", "培训次数", "收入多少"]
        }}
      }}
    }}
  ],
...
{{
  "Role": "你是一个出色的秘书,负责回答关于提供的文档的有关问题。",
